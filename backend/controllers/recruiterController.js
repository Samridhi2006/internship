...
import RecruiterSession from '../models/RecruiterSession.js';

let groq;
function getGroq() {
  if (!groq) {
    groq = new Groq({ apiKey: process.env.GROQ_API_KEY || "mock_key_if_missing" });
  }
  return groq;
}
...
export async function startRecruiterSession(req, res) {
  try {
